...
// URL FORZADA - IMAGEN DE LA PULGA SIN FILTROS
  ...
  return (
    <motion.div className="min-h-screen bg-black text-white p-6 font-sans flex flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-10 items-center justify-center max-w-7xl mx-auto w-full">
        {/* ESPECÍMEN DE LA PULGA - REMOVED GRAYSCALE AND FILTERS FOR NITIDEZ */}
        <div className="relative w-full lg:w-3/4 aspect-video rounded-[3.5rem] overflow-hidden border-2 border-white/5 bg-neutral-900 shadow-2xl flex items-center justify-center">
          <img 
            src={FLEA_IMAGE_URL} 
            alt="Pulga Especimen" 
            className={`w-full h-full object-contain transition-all duration-[1000ms] ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
            onLoad={() => setIsLoaded(true)}
            style={{ filter: 'none' }} // REMOVED FILTERS
          />
          ...
          {showDocMessage && (
            <motion.div initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 300, opacity: 0 }} className="w-full lg:w-80 p-8 bg-green-950/10 border-2 border-green-500/30 backdrop-blur-3xl rounded-[3rem] shadow-2xl relative">
               ...
               <p className="text-[11px] text-white/80 leading-relaxed italic mb-6">
                 "Nando, detectamos la pulga. Este <strong className='text-green-400'>VECTOR</strong> es el transporte de la peste. Cortar su cadena de transmisión es vital para la salud pública."
               </p>
